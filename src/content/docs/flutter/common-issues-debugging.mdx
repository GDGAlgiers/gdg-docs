...
<Card title="Common State Rebuild Problems">
  <Steps>
    1. **Excessive Rebuilds**
       Too many widgets rebuilding when only a small part of the UI needs updating.
    
    2. **Missing Rebuilds**
       UI not updating when the underlying data changes.
    
    3. **Inconsistent State**
       Different parts of the UI showing inconsistent data.
    
    4. **Build Context Issues**
       Errors related to using BuildContext after widget disposal.
  </Steps>
</Card>
    ...
    return Scaffold(
      body: Column(
        children: [
          ...
          // Only this actually needs to rebuild
          Text('Count: $_counter'),
...
class CounterScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: Column(
        children: [
          // Now this expensive widget doesn't rebuild
          ExpensiveWidget(),
...
<Card title="SelectionModel Pattern for Partial Updates">
...
class SelectableListItem extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    // Only listen to changes for this specific item
    final isSelected = selectionModel.isSelected(id);
    
    return ListTile(
      title: Text(title),
      selected: isSelected,
      onTap: () {
        // Toggle selection without rebuilding other items
        selectionModel.toggleSelection(id);
      },
    );
  }
}
...
class SelectionModel extends ChangeNotifier {
  ...
  // Value notifier for each item to avoid global rebuilds
  final Map<String, ValueNotifier<bool>> _selectionNotifiers = {};
  
  // Get or create a notifier for a specific ID
  ...
  void toggleSelection(String id) {
    ...
    // Also notify listeners for global state changes
    notifyListeners();
  }
...
<Card title="Identifying Layout Overflows">
...
<Tabs>
  <Tabs label="Row/Column Overflow">
    <Code code={`// PROBLEM: Text overflows in a fixed width Row
Row(
  children: [
    Icon(Icons.account_circle),
    Text(
      'This is a very long text that will overflow the available space',
    ),
  ],
)
...
Row(
  children: [
    ...
    Expanded(
      child: Text(
        'This is a very long text that will no longer overflow',
...
// SOLUTION 1: Use ListView.builder with a fixed height
Column(
  children: [
    Text('Header'),
    SizedBox(
...
// SOLUTION 3: Use a shrinkWrap ListView (less efficient but simpler)
Column(
  children: [
    Text('Header'),
    ListView.builder(
      ...
      itemCount: 5,  // Limit items for better performance
      itemBuilder: (context, index) => ListTile(
        title: Text('Item $index'),
      ),
    ),
  ...
  <Tabs label="Text Overflow">
    <Code code={`// PROBLEM: Text overflows its container
Container(
  width: 100,
  child: Text(
    'This is a long text that will overflow its container',
  ),
)

// SOLUTION 1: Use overflow property to handle overflow gracefully
Container(
  width: 100,
  child: Text(
    'This is a long text that will overflow its container',
...
// SOLUTION 2: Allow text to wrap to multiple lines
Container(
  width: 100,
  child: Text(
...
<Card title="Using LayoutBuilder for Responsive Layouts">
...
<Card title="Setting Up DevTools">
  <Steps>
    1. **Install DevTools**
       ```bash
    ...
    2. **Run Your App in Debug/Profile Mode**
       ```bash
    ...
    4. **Connect to Your App**
       Enter the URL from the Flutter run command into DevTools
  </Steps>
</Card>
...
<Card title="Key Performance Metrics to Watch">
  <Steps>
    1. **Frame Rendering Time**
       Look for frames that take longer than 16ms (60fps) to render.
    
    2. **CPU Usage**
       Identify CPU-intensive operations that might be causing stutters.
    
    3. **Memory Usage**
       Monitor for memory leaks and excessive object allocations.
    
    4. **Widget Rebuild Count**
...
<Card title="Monitoring Memory Leaks">
...
<Card title="Using Memory Snapshot">
  <Aside type="tip">
    DevTools' Memory page allows you to take memory snapshots. Look for:
    
    - Objects that unexpectedly persist across snapshots
    - Large numbers of the same object type
    - Steadily increasing memory usage during normal operation
  </Aside>
</Card>
...
<Card title="Techniques to Reduce Rebuilds">
  <Tabs>
    <Tabs label="Const Constructors">
...
Widget build(BuildContext context) {
  return const Container(
    ...
    <Tabs label="ValueNotifier & ValueListenableBuilder">
...
class CounterWidget extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    print('CounterWidget build'); // This runs only once
    
    return Column(
      children: [
        // This expensive widget doesn't rebuild when counter changes
        ExpensiveWidget(),
        ...
        ValueListenableBuilder<int>(
          ...
          builder: (context, value, child) {
            print('ValueListenableBuilder build'); // This runs on counter change
...
class _CounterPageState extends State<CounterPage> {
  ...
  @override
  Widget build(BuildContext context) {
    print('CounterPage build'); // This runs only once
    
    return Column(
      children: [
        // This expensive widget doesn't rebuild
        ExpensiveWidget(),
        ...
        StreamBuilder<int>(
          ...
          builder: (context, snapshot) {
            print('StreamBuilder build'); // This runs on counter change
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      <Aside type="tip">
        ...
        - Complex animations that don't need to trigger repaints elsewhere
        - Widget subtrees that update independently from the rest
        - Expensive rendering operations that don't change often
        
        But be cautious: excessive use can increase memory usage.
      </Aside>
    </Tabs>
  </Tabs>
</Card>
...
<Card title="Using Compute Function">
...
<Card title="Performance Best Practices Checklist">
  <Steps>
    1. **State Management**
...
<Aside type="tip">
  ### Performance Tips for Release Builds
