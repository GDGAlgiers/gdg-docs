...
import reactMockDataCodeExample from '../../code-examples/react/mock-data.jsx?raw';
...
<Code code={reactMockDataCodeExample} lang="jsx" title="SomeComponent.jsx"/>

Note that it's preferable to use `.js` or `.ts` formats for data files, instead of `.json`.

2. To keep your code ready for Backend integration, use the mock data in a state
```js
