...
<Aside type="note">
  Prisma uses a different approach where all models are defined in a single `schema.prisma` file instead of separate model files. This schema file is used to generate the Prisma client with strongly-typed queries.
</Aside>
